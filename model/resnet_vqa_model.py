...
class ResnetVQAModel(nn.Module):
    ...
    def __init__(
        ...
        if self.vision_model_name == "resnet34":
            self.vision_model = resnet34(pretrained=True)

        if self.vision_model_name == "resnet18":
        ...
        self.downscale_layer = nn.ConvTranspose2d(
            in_channels=2048,
            out_channels=768,
            kernel_size=3,  # Adjust kernel size as needed
            stride=1,       # Adjust stride as needed
    ...
    def forward(self, 
        ...
        if self.vision_model_name == "resnet34" or self.vision_model_name == "resnet18":
            self.vision_model.eval()
            with torch.no_grad():
                image_features = image_tensors
                for layer_name, resnet_block in self.vision_model._modules.items():
                    if layer_name not in ["avgpool","fc"]:
                        image_features = resnet_block(image_features.float()) 

        
            vision_embeddings = self.upscale_layer(image_features)

        elif self.vision_model_name == "resnet50":
            self.vision_model.eval()
            with torch.no_grad():
                image_features = image_tensors
                for layer_name, resnet_block in self.vision_model._modules.items():
            ...
            vision_embeddings = self.downscale_layer(image_features)
    ...
    def generate_answers(self,question_input_ids:torch.tensor, 
        ...
        if self.vision_model_name == "resnet34" or self.vision_model_name == "resnet18":
            ...
            vision_embeddings = self.upscale_layer(image_features)
            image_feature_maps_dict["features"] = image_features
