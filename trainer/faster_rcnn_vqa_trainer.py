...
from dataset_utils.resnet_vqa_daquar_dataset import DaquarFasterRcnnT5CollateFn, DaquarDataset
...
class FasterRcnnVQATrainer:
    ...
    def __init__(
        ...
        wandb.init(
            project="IDAI-610-Term-Project_09Dec_Resnet50_60_Epochs_RIT_GPU",
            config={
        ...
        def init_dataloader_helper(root_data_dir:str, 
            ...
            if type == "val":
                ...
                return DataLoader(
                    ...
                    collate_fn=DaquarFasterRcnnT5CollateFn(
    ...
    def _init_optimizer(self, optimizer_kwargs:dict, load_from_checkpoint:bool):
        ...
        param_dict.append({
            "params":self.model.lang_model.parameters(), "lr": optimizer_kwargs["lm_encoder_lr"], "model_name":"Language Model"
        })

        if self.model.vision_model_name == "resnet34" or self.model.vision_model_name == "resnet18" or self.model.vision_model_name == "faster-rcnn":

            param_dict.append({
                "params":self.model.upscale_layer.parameters(), "lr": 0.0005, "model_name":"UpScaler Layer"
            })        

        elif self.model.vision_model_name == "resnet50":
            param_dict.append({
                "params":self.model.downscale_layer.parameters(), "lr": 0.0005, "model_name":"DownScaler Layer"
            })        

        param_dict.append({
            "params":self.model.sga_modules.parameters(), "lr": 0.0005, "model_name":"Self-Guided Attention Module"
        })        

        param_dict.append({
            "params":self.model.attention_pooler.parameters(), "lr": 0.0005, "model_name":"Attention Pooler"
        })        
    ...
    def valid_one_epoch(self):
        ...
        with torch.no_grad():
            for epoch, data_items in enumerate(self.test_dataloader):
                ...
                valid_questions.extend(data_items["questions"])
        ...
        for prediction, target, question in zip(valid_predictions, valid_targets, valid_questions):
        ...
        if avg_wups_score > self.callbacks.best_score:
            ...
            self.callbacks.save_checkpoint(self.model, self.cur_epoch, self.test_dataloader.collate_fn.answer_spaces)
        ...
        if not os.path.exists(f'{self.output_dir}/validation_predictions'):
            os.makedirs(f'{self.output_dir}/validation_predictions')

        with open(f'{self.output_dir}/validation_predictions/{self.cur_epoch}_validation_predictions.json','w+') as f:
            json.dump(predictions_dict_list, f)
