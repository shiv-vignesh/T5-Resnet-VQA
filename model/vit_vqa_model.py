...
from transformers import T5ForConditionalGeneration, T5Config, ViTModel, RobertaModel, ViTConfig
...
class VitVQAModel(nn.Module):
    ...
    def generate_answers(self,question_input_ids:torch.tensor, 
        ...
        if self.vision_model_name == "google/vit-base-patch16-224-in21k":
            with torch.no_grad():
                vision_embeddings = self.vision_model(pixel_values, output_attentions=True)
                pooled_outputs = vision_embeddings.pooler_output
                attention_tensors = vision_embeddings["attentions"]
        ...
        if annotation_ids != None:   
            ...
            return lm_logits, loss, attention_tensors
        
        else:
            return lm_logits, None, attention_tensors



    def generate_answers_beam(self, question_input_ids:torch.tensor, 
                decoder_question_input_ids:torch.tensor,
                question_attention_masks:torch.tensor, 
                decoder_question_attention_masks:torch.tensor,
                annotation_ids:torch.tensor,
